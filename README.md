...
- 绑定普通网页、MediaWiki、百科页面或资料站链接，并交给配置的网页抓取 API 与整理 API 处理。
- 选择使用兼容聊天补全格式的第三方 API，或使用酒馆当前主 API 做 AI 整理。
...
5. 选择 API 来源：
   - **第三方 API**：抓取服务返回网页原文，插件再调用你配置的兼容聊天补全 API 整理。
   - **酒馆主 API**：外部服务只返回网页原文，插件再调用当前 SillyTavern 主 API 整理。
6. 根据需要填写网页抓取 API 地址、聊天补全 API 地址、API Key、模型列表 API、模型、语言和 token 预算。
7. 可以在 **整理 API** 面板里点击 **获取模型**、**测试连接**、**测试上下文**，确认后点 **保存设置**。
...
网页抓取 API 会先接收来源列表并返回原文。插件会向 `网页抓取 API 地址` 发送 `POST` 请求：
...
如果填写了 `抓取 API Key`，插件会带上：

```text
Authorization: Bearer <API Key>
```

抓取 API 可以直接返回整理后的文本，也可以返回原文。兼容以下任意一种字段：
...
插件会优先读取 `prompt`，也兼容 `context`、`content`、`text`。如果配置了聊天补全 API 地址，插件会把 `raw`、`rawText`、`sourceText`、`content`、`text` 或 `entries` 整理出的原文发送到第三方 API 的 `/chat/completions` 兼容接口。

## 第三方 API 模式

选择 **第三方 API** 时，`聊天补全 API 地址` 可以填写 base URL，例如：
...
- `POST /chat/completions`：整理网页原文为 lore。
- `GET /models`：点击 **获取模型** 时使用；如果你填写了 `模型列表 API`，则优先使用你填写的完整地址。

如果你的服务不使用标准路径，也可以直接把完整聊天补全地址填进 `聊天补全 API 地址`，例如 `https://api.example.com/v1/chat/completions`；模型列表则填完整的 `模型列表 API`。

如果填写了 `API Key`，第三方 API 请求会带上：

```text
Authorization: Bearer <API Key>
```

如果不填写 `聊天补全 API 地址`，插件会保留 0.4.5 以前的旧版一体化整理 API 协议：直接把来源列表发给 `网页抓取 API 地址`，并读取它返回的 `prompt` / `context` / `entries`。
...
点击 **获取模型** 时，如果填写了 `模型列表 API`，插件会请求该地址；如果没填，会尝试从聊天补全 API 地址推导 `/models`。酒馆主 API 模式使用当前聊天模型，不需要获取模型列表。
...
- 外部网页 / Wiki 来源的权限、登录态、反爬和抓取规则由网页抓取 API 处理。
- 绑定别人发来的 OC Wiki 分享链接时，只能读取那个分享链接允许读取的条目。

## 从旧版升级

0.3.0 起，扩展显示名改为 **External Lore Source**，但会自动迁移旧的 `OC Wiki Worldbook` 设置。0.4.0 新增整理 API 弹窗、模型获取、测试连接和酒馆主 API 模式。0.4.1 会在新版脚本加载时替换已经存在的旧版设置面板，避免扩展热更新后仍显示旧表单。0.4.2 把来源绑定、API、注入设置和测试入口都收进同一个设置弹窗，并在世界书页面增加快捷按钮。0.4.3 会在扩展设置面板渲染前先准备弹窗，并把世界书页入口改成明显的“外置 Lore”按钮。0.4.4 提高设置弹窗的主题背景和文字对比度，改善浅色/透明主题下的可读性。0.4.5 修复弹窗底部按钮在部分主题/宽度下被压成单字竖排的问题，并兼容 Persona Weaver 使用的 `TavernHelper.generateRaw` 主 API 调用方式。0.4.6 把 API 来源改成主 API 风格的下拉选择，支持兼容聊天补全格式的第三方自定义 API，测试/获取模型按钮移入整理 API 面板，并让设置弹窗只挂到页面顶层。

如果扩展列表显示已经更新，但设置面板仍然是旧界面：

1. 在扩展管理器里再执行一次更新，确认版本至少是 `0.4.6`。
2. 重启 SillyTavern 后端，然后在浏览器里硬刷新页面。
3. 检查 `public/scripts/extensions/third-party/` 下是否有多个旧插件目录；如果有，只保留当前安装器使用的那个。
4. 新版面板底部状态会显示 `待命 · v0.4.6`。
...
Version 0.4.6 switches API source selection to a main-API-style selector, supports custom chat-completion-compatible API URLs, moves API tests into the API panel, and mounts the settings modal above the World Info page.
